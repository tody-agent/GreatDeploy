    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                HStack { Text("Accounts").font(.title2).fontWeight(.bold); Spacer(); Button(action: { selectedItem = .addAccount }) { Label("Add", systemImage: "plus") }.buttonStyle(.borderedProminent) }
                if accountStore.accounts.isEmpty { Text("No accounts yet").foregroundStyle(.secondary).frame(maxWidth: .infinity).padding() }
                else {
                    ForEach(accountStore.accounts) { account in
                        HStack(spacing: 16) {
                            ...
                            Spacer()
                            Button(action: { selectedItem = .account(account) }) { Image(systemName: "pencil.circle").font(.title2).foregroundStyle(.blue) }.buttonStyle(.plain)
                        }.padding(.horizontal, 16).padding(.vertical, 12).background(Color(nsColor: .controlBackgroundColor)).clipShape(RoundedRectangle(cornerRadius: 12)).onTapGesture { if !account.isActive { Task { await performAccountSwitch(to: account, accountStore: accountStore, isSwitching: $isSwitching, showNotification: showNotificationOnSwitch) } } }
                    }
                }
            }.padding(30).frame(maxWidth: .infinity, alignment: .leading)
        }.background(Color(nsColor: .textBackgroundColor))
