...
final class CodexAdapterTests: XCTestCase {
    ...
    private func makeAdapter(tempDir: URL) -> (adapter: CodexAdapter, configPath: URL) {
    ...
    private func fixtureContent() throws -> String {
        let bundle = Bundle(for: CodexAdapterTests.self)
    ...
    func test_writeServers_preservesUserAddedServers() throws {
        ...
        let writtenServers = try TOMLSerializer.parse(written)
        let writtenNames = Set(writtenServers.map(\.name))

        XCTAssertTrue(writtenNames.contains("user-codex-server"), "User-added server should be preserved")
    ...
    func test_writeServers_removesOrphans() throws {
        ...
        try adapter.writeServers(
            bundleServers,
            existingContent: fixtureContent,
            previouslySyncedNames: ["filesystem", "user-codex-server"]
        )
        ...
        XCTAssertFalse(writtenNames.contains("user-codex-server"), "Orphan should be removed")
