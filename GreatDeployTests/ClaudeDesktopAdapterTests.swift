...
final class ClaudeDesktopAdapterTests: XCTestCase {
    ...
    private func makeAdapter(tempDir: URL) -> (adapter: ClaudeDesktopAdapter, configPath: URL) {
    ...
    private func fixtureContent() throws -> String {
        let bundle = Bundle(for: ClaudeDesktopAdapterTests.self)
        guard let url = bundle.url(forResource: "claude_desktop_config", withExtension: "json") else {
