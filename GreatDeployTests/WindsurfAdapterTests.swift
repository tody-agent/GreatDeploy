...
final class WindsurfAdapterTests: XCTestCase {

    // MARK: - Helpers

    private func makeAdapter(tempDir: URL) -> (adapter: WindsurfAdapter, configPath: URL) {
        let configPath = tempDir.appendingPathComponent("mcp_config.json")
    ...
    private func fixtureContent() throws -> String {
        let bundle = Bundle(for: WindsurfAdapterTests.self)
        guard let url = bundle.url(forResource: "windsurf_mcp_config", withExtension: "json") else {
    ...
    func test_readServers_parsesFixture_intoTwoServers() throws {
        ...
        XCTAssertTrue(names.contains("user-windsurf-server"))
    ...
    func test_writeServers_preservesUserAddedServers() throws {
        ...
        XCTAssertTrue(writtenNames.contains("user-windsurf-server"), "User-added server should be preserved")
    ...
    func test_writeServers_removesOrphans() throws {
        ...
        try adapter.writeServers(
            bundleServers,
            existingContent: fixtureContent,
            previouslySyncedNames: ["filesystem", "user-windsurf-server"]
        )
        ...
        XCTAssertFalse(writtenNames.contains("user-windsurf-server"), "Orphan should be removed")
    ...
    func test_writeServers_preservesNonMCPSettings() throws {
        ...
        XCTAssertEqual(json?["autoUpdate"] as? Bool, true, "autoUpdate should be preserved")
