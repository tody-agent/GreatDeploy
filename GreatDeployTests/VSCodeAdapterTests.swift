...
final class VSCodeAdapterTests: XCTestCase {
    ...
    private func makeAdapter(tempDir: URL) -> (adapter: VSCodeAdapter, configPath: URL) {
    ...
    private func fixtureContent() throws -> String {
        let bundle = Bundle(for: VSCodeAdapterTests.self)
        guard let url = bundle.url(forResource: "vscode_settings", withExtension: "json") else {
    ...
    func test_readServers_parsesFixture_intoTwoServers() throws {
        ...
        let servers = try JSONAdapterHelpers.parseServers(from: content, keyPath: ["mcp", "servers"])
        XCTAssertEqual(servers.count, 2)

        let names = Set(servers.map(\.name))
        XCTAssertTrue(names.contains("filesystem"))
        XCTAssertTrue(names.contains("user-vscode-server"))
    ...
    func test_readServers_emptyFile_returnsEmptyArray() throws {
        let servers = try JSONAdapterHelpers.parseServers(from: "", keyPath: ["mcp", "servers"])
    ...
    func test_writeServers_preservesUserAddedServers() throws {
        ...
        let writtenServers = try JSONAdapterHelpers.parseServers(from: written, keyPath: ["mcp", "servers"])
        let writtenNames = Set(writtenServers.map(\.name))

        XCTAssertTrue(writtenNames.contains("user-vscode-server"), "User-added server should be preserved")
    ...
    func test_writeServers_removesOrphans() throws {
        ...
        try adapter.writeServers(
            bundleServers,
            existingContent: fixtureContent,
            previouslySyncedNames: ["filesystem", "user-vscode-server"]
        )
        ...
        XCTAssertFalse(writtenNames.contains("user-vscode-server"), "Orphan should be removed")
    ...
    func test_writeServers_preservesNonMCPSettings() throws {
        ...
        XCTAssertEqual(json?["editor.theme"] as? String, "dark-plus", "editor.theme should be preserved")
        XCTAssertEqual(json?["editor.fontSize"] as? Int, 14, "editor.fontSize should be preserved")
