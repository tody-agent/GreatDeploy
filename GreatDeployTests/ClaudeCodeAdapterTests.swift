...
final class ClaudeCodeAdapterTests: XCTestCase {
    ...
    private func makeAdapter(tempDir: URL) -> (adapter: ClaudeCodeAdapter, configPath: URL) {
    ...
    private func fixtureContent() throws -> String {
        let bundle = Bundle(for: ClaudeCodeAdapterTests.self)
        guard let url = bundle.url(forResource: "claude_settings", withExtension: "json") else {
    ...
    func test_readServers_parsesFixture_intoTwoServers() throws {
        ...
        XCTAssertTrue(names.contains("user-claude-server"))
    ...
    func test_writeServers_preservesUserAddedServers() throws {
        ...
        XCTAssertTrue(writtenNames.contains("user-claude-server"), "User-added server should be preserved")
    ...
    func test_writeServers_removesOrphans() throws {
        ...
        try adapter.writeServers(
            bundleServers,
            existingContent: fixtureContent,
            previouslySyncedNames: ["filesystem", "user-claude-server"]
        )
        ...
        XCTAssertFalse(writtenNames.contains("user-claude-server"), "Orphan should be removed")
